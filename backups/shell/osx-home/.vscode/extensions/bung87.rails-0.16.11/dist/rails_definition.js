...
function getConcernsFilePath(lineStartToWord, fileT) {
    ...
    const concern = lineStartToWord.replace(constants_1.PATTERNS.INCLUDE_DECLARATION, ''), seq = concern.split('::').map(wordsToPath);
    if (seq[0] === 'concerns')
        delete seq[0];
...
async function getLibOrModelFilePath(document, lineStartToWord, word) {
    ...
    thePath = sub ? path.join(root, 'lib', sub + '.rb') : '', demodulized = inflection.demodulize(symbol), funcOrClass = demodulized.indexOf('.') !== -1 ? demodulized.split('.')[1] : demodulized, regPrefix = constants_1.PATTERNS.CAPITALIZED.test(funcOrClass)
        ? 'class\\s+'
    ...
    try {
        const uris = await utils_1.findFiles(document, filePathInModels, null, 1);
        if (!uris.length) {
