...
class DataSource {
    ...
    pullBranch(repo, branchName, remote, createNewCommit, squash) {
        ...
        return this.runGitCommand(args, repo).then((pullStatus) => {
            return pullStatus === null && squash
                ? this.commitSquashIfStagedChangesExist(repo, remote + '/' + branchName, "Branch", config_1.getConfig().squashPullMessageFormat)
                : pullStatus;
        });
    }
    ...
    merge(repo, obj, actionOn, createNewCommit, squash, noCommit) {
        ...
        return this.runGitCommand(args, repo).then((mergeStatus) => {
            return mergeStatus === null && squash && !noCommit
                ? this.commitSquashIfStagedChangesExist(repo, obj, actionOn, config_1.getConfig().squashMergeMessageFormat)
                : mergeStatus;
        });
    }
    rebase(repo, obj, actionOn, ignoreDate, interactive) {
        if (interactive) {
            return this.openGitTerminal(repo, 'rebase --interactive ' + (actionOn === "Branch" ? obj.replace(/'/g, '"\'"') : obj), 'Rebase on "' + (actionOn === "Branch" ? obj : utils_1.abbrevCommit(obj)) + '"');
        }
        else {
            let args = ['rebase', obj];
            if (ignoreDate)
    ...
    commitSquashIfStagedChangesExist(repo, obj, actionOn, squashMessageFormat) {
